...
Native Android app built with Kotlin and Jetpack Compose. Uses the domain model directly (resolved from Maven local repository) with in-memory storage — no backend required to run the app.
...
- Domain model from `budinv-model` (Maven local, in-memory storage)
- MVVM with StateFlow
...
**Prerequisites:**
- Android SDK — install via [Android Studio](https://developer.android.com/studio) and set `ANDROID_HOME`
- **Java 21** (the Kotlin toolchain is incompatible with Java 25; use sdkman or similar to switch)
- `budinv-model` published to your local Maven repository (one-time step):

```bash
mvn install -N -DskipTests          # install parent POM
mvn install -pl budinv-model -DskipTests
```
